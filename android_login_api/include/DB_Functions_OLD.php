...
class DB_Functions {
     ...
     * returns details
     */
    public function addUserBank($uid,$cardNumber,$cardValidity,$cardCVV,$cardName,$cardType){
        ...
        if(mysqli_query($conn,$bank)){
            ...
            // return false;
            $response['error'] = TRUE;
            $response['error_msg'] = ' This CARD Already Exist !';
     ...
     * returns details
     */
    public function updateUserBank($uid,$cardNumber,$cardValidity,$cardCVV,$cardName){
        date_default_timezone_set('Asia/Calcutta'); 
    ...
    */
    public function getUserBank($uid){
        ...
        $result = mysqli_query($conn,"SELECT `card_number`, `card_validity`, `card_name`, `card_type` FROM `tbl_bank_details` WHERE `unique_id` = '$uid'");
    ...
    public function getHistoryPayment($UID,$indexID){
        ...
        $curr = "SELECT slot_id as SlotNumber,bid,amount,status as status,created_at FROM `tbl_bid_transaction` WHERE u_id = '$UID' LIMIT $initial,$limit";
        $result = mysqli_query($conn,$curr) or die(mysqli_error());
   ...
   /*
    ...
    public function searchHistoryPayment($UID,$IndexID,$fromDate,$toData){
        ...
        $Initial = $IndexID.'0';
        $sear =  "SELECT slot_id as SlotNumber,bid,amount,status as status,created_at FROM `tbl_bid_transaction` WHERE u_id = '$UID' AND  created_at >= '$fromDate' AND created_at <= '$toData' LIMIT $Initial,10";       
        $result = mysqli_query($conn,$sear) or die(mysqli_error());
...
*********/
    ...
    public function GetPayUMoneyWithDrawl(){
        ...
        $allUr = "SELECT * FROM `tbl_payumoney_transaction`";
        $result = mysqli_query($conn,$allUr) or die(mysqli_error());
